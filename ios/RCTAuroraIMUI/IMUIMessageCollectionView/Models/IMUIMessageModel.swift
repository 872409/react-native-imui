...
//      }
        ...
        case .voice:
            ...
            break
        case .rtc_call:
            var strContent = self.customDict.object(forKey: "content");
            if ( strContent == nil || !(strContent is String)) {
                let contentKey = self.isOutGoing ? "outgoing" : "incoming"
                strContent = self.customDict.object(forKey: contentKey) as! String
            }
            
            let tmpLabel = YYLabel()
            tmpLabel.setupYYText(strContent as! String, andUnunderlineColor: UIColor.white)
            tmpLabel.font = IMUITextMessageCell.inComingTextFont
            bubbleContentSize = tmpLabel.getTheLabelBubble(CGSize(width: IMUIMessageCellLayout.bubbleMaxWidth, height: 40))
            bubbleContentSize.width += 40
//            bubbleContentSize = CGSize(width: 120, height: 40)
            break
        case .video:
